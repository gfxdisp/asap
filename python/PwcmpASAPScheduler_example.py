...
for kk in range(N_batch): # % for each pairwise comparison in the batch
    ...
    print( 'Compare {} with {}\n'.format(condition_table['method'][stim_A], condition_table['method'][stim_B]) )
    ...
    # i_a = np.where( condition_table['method'][stim_A] == np.array(method) )[0][0]
    # i_b = np.where( condition_table['method'][stim_B] == np.array(method) )[0][0]    

    # p_sel = norm.cdf( i_b-i_a )
    is_A_selected = np.random.randint(2)
