        ...
        if not os.path.exists( self.result_file ):
            ...
            for kk in range(self.N_blocks): # % for each block
                ...
                for rr in range(Rss.shape[0]):
                    ind_A = np.where(utils.columns_equal( Dss, non_block_columns, Rss[non_block_columns_A].iloc[[rr]].to_numpy()[0] ) > 0)[0]
                    ind_B = np.where(utils.columns_equal( Dss, non_block_columns, Rss[non_block_columns_B].iloc[[rr]].to_numpy()[0] ) > 0)[0]
