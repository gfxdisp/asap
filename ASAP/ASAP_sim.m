function [M] = ASAP_sim(q,n_comp,mode,M)
    ...
    while comp<n_comp
        [inf_mat,init_data,pairs_to_compare]=KLD_mat(N,init_data);
        if strcmp(mode,'seq')
            G = simulate_observer_choice(q,pairs_to_compare(1),pairs_to_compare(2),G);
            comp=comp+1;
        else
