function [kl_divs,out_data,arg] = KLD_mat(Nc,init_data)
    ...
    for ii=2:Nc
        for jj=1:(ii-1)
            ...
            if init_data.prob_cmps(ii,jj)>rand()
                out_data.G = [G; ii,jj];
                [out_data_iijj]=ts_offline(out_data);
                kl1=norm_kl(out_data_iijj.Ms,out_data.Ms,out_data_iijj.Vs,out_data.Vs);

                out_data.G = [G; jj,ii];
                [out_data_jjii]=ts_offline(out_data);
                kl2=norm_kl(out_data_jjii.Ms,out_data.Ms,out_data_jjii.Vs,out_data.Vs);
